...
std::string RAMDetector::read_file(const std::string& path) const {
...
std::unordered_map<std::string, uint64_t> RAMDetector::parse_meminfo() const {
...
RAMProcessInfo RAMDetector::get_process_memory_info(uint32_t pid) const {
    RAMProcessInfo info;
    info.pid = pid;
