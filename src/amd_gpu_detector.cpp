...
class AMDGPUDetector::Impl {
private:
    bool initialized_;
    std::vector<std::string> gpu_paths_;
    ...
    bool is_amd_gpu(const std::string& path) {
        ...
        return vendor.find("0x1002") != std::string::npos;  // AMD vendor ID
    }
    ...
    std::vector<GPUInfo> get_gpu_info() const {
        ...
        for (const auto& path : gpu_paths_) {
            ...
            if (!vram_total.empty() && !vram_used.empty()) {
                info.total_memory_mb = std::stoull(vram_total) / (1024.0 * 1024.0);
                info.used_memory_mb = std::stoull(vram_used) / (1024.0 * 1024.0);
            }
        ...
        for (const auto& entry : std::filesystem::directory_iterator("/proc")) {
            ...
            if (process_match) {
                ...
                while (std::getline(maps_file, line)) {
                    if (line.find("/dev/dri/") != std::string::npos || 
                    ...
                    if (std::filesystem::exists(fd_path)) {
                        for (const auto& fd : std::filesystem::directory_iterator(fd_path)) {
                            if (std::filesystem::is_symlink(fd.path())) {
                                ...
                                if (!ec && (target.find("/dev/dri/") != std::string::npos || 
                                          target.find("amdgpu") != std::string::npos)) {
                                    uses_gpu = true;
                                    gpu_path = target;
                    ...
                    while (std::getline(maps_file, line)) {
                        if (line.find("/dev/dri/renderD128") != std::string::npos) {
                            ...
                            if (dash_pos != std::string::npos) {
                                ...
                                try {
                                    uint64_t start = std::stoull(start_addr, nullptr, 16);
                                    uint64_t end = std::stoull(end_addr, nullptr, 16);
                                    total_gpu_mem += (end - start);
                                } catch (...) {}
                            }
                        }
                    }
                    proc_info.memory_usage_mb = total_gpu_mem / (1024.0 * 1024.0);
                        ...
                        if (gpu_path.find(std::filesystem::path(path).filename().string()) != std::string::npos) {
                            ...
                            if (std::regex_search(card_name, match, card_regex)) {
                                ...
                                if (!vram_used.empty() && !vram_total.empty()) {
                                    try {
                                        float used_mb = std::stoull(vram_used) / (1024.0 * 1024.0);
                                        float total_mb = std::stoull(vram_total) / (1024.0 * 1024.0);
...
AMDGPUDetector::AMDGPUDetector() : pimpl_(std::make_unique<Impl>()) {}
AMDGPUDetector::~AMDGPUDetector() = default;
